...
/* Table Updated -> OCIDWSTG.STOREKEEPER_CATALOG_STG */
function load_storekeeper_catalog_stg(p_file_name in varchar2) return varchar2 is
  v_dt     date;
  v_file   bfile;
  v_cnt    integer;
begin
   ...
   delete from ocidwstg.storekeeper_catalog_stg where weekday = trim(to_char(v_dt,'DAY'));
   
   insert into ocidwstg.storekeeper_catalog_stg (
      dw_dataset_date,
      part_no,
      type,
      platform,
   ...
   select 
      ...
      JSON_TABLE(v_file,
        '$.data[*]' columns (
           ...
           manufacturer_name     varchar2(512)   path '$.Manufacturer_Name'          null on error,
           manufacturer_part     varchar2(512)   path '$.Manufacturer_Part'          null on error,
...
exception
  when others then
    rollback;
    raise;
end load_storekeeper_catalog_stg;

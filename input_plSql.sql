create or replace package ocidwstg.storekeeper_stg_pkg is
/* Current version 1::2020-04-15::pceschim::BIZINT-2745  First checkin for new clean-and-green dataset */
function load_storekeeper_regions_stg(p_file_name in varchar2) return varchar2;

end storekeeper_stg_pkg;
/

create or replace package body ocidwstg.storekeeper_stg_pkg is
/* Current version 1::2020-04-15::pceschim::BIZINT-2745  First checkin for new clean-and-green dataset */
...
function load_storekeeper_regions_stg(p_file_name in varchar2) return varchar2 is
  ...
  v_cnt    integer;
begin
   v_dt   := to_date(regexp_substr(p_file_name,'\d{8}'),'YYYYMMDD');
   ...
   delete from ocidwstg.storekeeper_regions_stg
   where weekday = to_char(v_dt,'DAY');
